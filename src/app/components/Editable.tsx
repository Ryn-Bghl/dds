...
export const Editable: React.FC<EditableProps> = ({
  ...
  if (isEditing) {
    return (
      <div className={cn("relative border-2 border-[#F29F05] p-2 rounded-md bg-black/50 z-50", className)}>
        {label && <label className="text-xs text-[#F29F05] mb-1 block">{label}</label>}
        {type === 'textarea' ? (
          <Textarea
            value={tempValue}
        ...
        <div className="flex gap-2 mt-2 justify-end">
          <Button size="sm" variant="ghost" onClick={handleCancel} className="text-gray-400 hover:text-white">
            <X className="w-4 h-4" />
          </Button>
          <Button size="sm" className="bg-[#F29F05] text-black hover:bg-[#D96704]" onClick={handleSave}>
