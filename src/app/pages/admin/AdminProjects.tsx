...
import { Plus, Trash2, Edit3, Save, X, Music } from 'lucide-react';
...
export default function AdminProjects() {
  ...
  const saveEdit = () => {
    if (editForm) {
      const newProjects = content.projects.map(p => p.id === editForm.id ? editForm : p);
    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400">Ajoutez, modifiez ou supprimez les projets de l'association.</p>
        </div>
      ...
      <div className="grid gap-6">
        {content.projects.map((project) => (
          <Card key={project.id} className="bg-[#1a1a1a] border-gray-800 overflow-hidden">
              ...
              <div className="flex items-center p-4 gap-4">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-[10px] bg-gray-800 px-2 py-0.5 rounded text-gray-400 uppercase tracking-wider">{project.category}</span>
                  </div>
                  <p className="text-sm text-gray-400 line-clamp-2">{project.description}</p>
                </div>
                <div className="flex gap-2">
                  <Button variant="ghost" size="icon" onClick={() => startEdit(project)} className="text-blue-400 hover:text-blue-300">
